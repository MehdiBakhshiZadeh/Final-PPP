...
\begin{enumerate}
    ...
    \item \textbf{Test mode:} execution of the main verification suite. The entry script is\\
     \texttt{tests/run\_all\_tests.m}.
    \item \textbf{Benchmark mode:} performance comparison of solver variants. The entry script is \texttt{tests/benchmark\_solvers.m}.
\end{enumerate}
...
The project is provided as a normal folder-based MATLAB project and is intended to be used directly after unpacking. No separate installer is required. To prepare the program for use, unzip the project folder, place it in a suitable location, and open MATLAB.
...
The numbered elements in Figure~\ref{fig:manual_gui_overview} have the following meaning:

...
\paragraph{MATLAB version note.}
The project was developed and tested with MATLAB 2025b. Older MATLAB versions may still run parts of the program, but compatibility is not guaranteed. In particular, GUI theme-related properties may cause exceptions in older releases, and possible incompatibilities in other parts of the program cannot be excluded. For reliable execution, MATLAB 2025b or newer is recommended.
...
\begin{enumerate}
    ...
    \item \textbf{Non-GUI mode:} scripted execution through \path{run_model_class.m} for reproducible runs.
    \item \textbf{Test mode:} execution of the main verification suite. The entry script is \path{tests/run_all_tests.m}.
    \item \textbf{Benchmark mode:} performance comparison of solver variants. The main benchmark script is in the benchmark folder:
    
    \path{tests/benchmark_solver_performance/benchmark_solvers.m}
\end{enumerate}
...
The project is provided as a standard folder-based MATLAB project and is intended to be used directly after unpacking. No separate installer is required. To prepare the program for use, unzip the project folder, place it in a suitable location, and open MATLAB.

Two practical ways of starting the project are supported:

\begin{enumerate}
    \item \textbf{Start from the project root folder.} Navigate in MATLAB to the root folder of the unpacked project and run one of the starter scripts such as \texttt{run\_gui} or \texttt{run\_model\_class}.
    \item \textbf{Make the project root callable from MATLAB.} If the user does not want to work directly from the project root, it is sufficient to add the project root folder, which contains the starter scripts, to the MATLAB path and then call \texttt{run\_gui} or \texttt{run\_model\_class} from there. The starter scripts set the required paths to the numerical kernel automatically.
\end{enumerate}

The program should be regarded as a standalone MATLAB project, not as an installable MATLAB app. In particular, packaging it through the MATLAB app installer is not the intended usage mode of the final submission, because such an installation workflow may not reproduce the required internal paths reliably on another system.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/manual_gui_annotated_no_legend.png}
    \caption{Annotated screenshot of the final GUI during a typical startup state. The numbered callouts indicate the main elements used during interactive operation.}
    \label{fig:manual_gui_overview}
\end{figure}

Figure~\ref{fig:manual_gui_overview} shows the practical arrangement of the two-window GUI during normal use. The upper window is the separate MATLAB plot window, while the lower window is the controller interface. This layout allows the user to keep simulation control and visualization separated during interactive work. In addition, the separate plot window gives direct access to MATLAB's standard figure tools, including the usual save-figure functionality.

The numbered elements in Fig.~\ref{fig:manual_gui_overview} have the following meaning:

\begin{enumerate}
    \item \textbf{Plot window:} displays the currently selected solution field during initialization and simulation.
    \item \textbf{Reaction-parameter sliders:} used to adjust the Gray--Scott parameters \(F\) and \(k\) interactively.
    \item \textbf{Boundary-condition controls:} used to choose between periodic, Neumann, and Dirichlet boundary conditions.
    \item \textbf{Run-control buttons:} used to reset, restart, record, pause, and run the simulation.
    \item \textbf{Field and video settings:} used to choose the displayed field and the recording settings.
    \item \textbf{Status bar:} shows the current simulation state, including time step, displayed field, boundary-condition mode, parameter values, and run status.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \path{test_matrix_stencil_endtoend_equivalence}\\
    end-to-end comparison between matrix and stencil solver paths.
\end{itemize}
...
The folder \texttt{tests/test\_case\_06\_gui\_nongui\_consistency/} contains a separate workflow to verify that the GUI reproduces the same numerical results as the non-GUI execution path. This check is not part of \texttt{run\_all\_tests}, because it requires a dedicated comparison workflow. The recommended procedure is:

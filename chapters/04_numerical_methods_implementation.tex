\chapter{Numerical Methods and Implementation Details}
...
\begin{quote}
\small\ttfamily
defaultParams $\rightarrow$ finalizeParams $\rightarrow$ buildGrid\\
makeOperator $\rightarrow$ GrayScottModel $\rightarrow$ eulerStep
\end{quote}
...
After one explicit Euler step, \texttt{applyBoundaryConditions.m} reshapes the state vectors into two-dimensional arrays:
\[
...
If the mode is set to \texttt{stencil}, no global Laplacian matrix is stored. Instead, the code builds a compact stencil description in \texttt{buildStencil2D.m}. This structure contains only the information needed to evaluate the five-point stencil directly on the grid. During each step, \texttt{applyDiffusion.m} calls \texttt{applyLaplacianStencil.m}. That routine reshapes the vectorized state into a two-dimensional array, forms the neighboring values by periodic shifts, evaluates the stencil locally, and then vectorizes the result again. This backend is therefore matrix-free: it applies the same discrete operator without assembling the full global matrix.
...
\subsection{GUI design evolution and final concept}
\label{subsec:gui_evolution}
...
The first concept was an all-in-one interface in which the simulation controls and the graphical visualization were planned to appear in a single window. This idea was attractive because it grouped all user interactions and outputs in one place. Fig.~\ref{fig:gui_mockup_allinone} shows this early mockup. However, during implementation this approach became less suitable, mainly because plotting inside an embedded axis object caused practical difficulties. In addition, combining all controls and visualization elements in one window made the interface more crowded and harder to manage.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/mockup-all-in-one.png}
    \caption{Early all-in-one GUI mockup combining controls and simulation visualization in a single window.}
    \label{fig:gui_mockup_allinone}
\end{figure}

For this reason, the design was revised toward a two-window concept. In this second approach, one window was dedicated to simulation control and parameter adjustment, while a separate window was used for visualization of the computed field. An early mockup of this direction is shown in Fig.~\ref{fig:gui_mockup_twowindows}. This change reduced clutter, improved usability, and made the MATLAB implementation more manageable.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/mockup-two-windows.png}
    \caption{Revised GUI mockup with separated controller and visualization windows.}
    \label{fig:gui_mockup_twowindows}
\end{figure}
...
\subsection{Final GUI structure and main features}
\label{subsec:gui_structure}
...
The controller window is shown in Fig.~\ref{fig:gui_final_controller}. It combines the most important simulation controls in one place. The interface includes parameter sliders for the feed and kill rates, displayed diffusion coefficients, boundary-condition selection, and the main execution buttons for running, pausing, restarting, resetting, and recording the simulation. In addition, the user can choose which field is displayed, select a video format for export, define the recording frame rate, and monitor the current simulation state through the status display. The equations are also shown directly in the interface so that the GUI remains closely connected to the underlying mathematical model.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/final-desing-controller.png}
    \caption{Final controller window of the Gray--Scott GUI.}
    \label{fig:gui_final_controller}
\end{figure}

The visualization itself is handled in a separate plot window, shown in Fig.~\ref{fig:gui_final_plotter}. This window focuses on displaying the selected simulation field in a clean and readable form. By separating plotting from parameter control, the graphical output remains less cluttered and easier to inspect during interactive runs. This also improves the practical usability of the program, since the user can focus on simulation control in one window and on pattern development in another.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/final-design-plotter.png}
    \caption{Separate plot window used for visualization of the selected solution field.}
    \label{fig:gui_final_plotter}
\end{figure}

Even at a high level, the GUI is more than a simple collection of buttons and sliders. User actions such as moving a slider, choosing a preset, loading a saved model, starting or pausing a run, or enabling recording must trigger coordinated updates in several parts of the program. In particular, GUI controls, the shared model state, and the visualization components must remain synchronized during interactive use. The overall interaction logic is illustrated by the information-flow diagram in Fig.~\ref{fig:gui_information_flow}. This makes clear that an important part of the programming effort was invested in the interaction logic behind the interface, not only in the visual arrangement of GUI elements.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/InformationFlowGUI.png}
    \caption{Information flow between the main GUI elements, the shared model state, and the visualization components.}
    \label{fig:gui_information_flow}
\end{figure}

In this way, the final GUI provides an interface that is practical for interactive use while remaining closely connected to the numerical model. The two-window architecture not only reduces visual clutter, but also gives the user direct access to MATLAB's standard figure tools in the separate plot window, including the save-figure functionality. This makes the application easier to use, easier to maintain, and well suited to repeated simulation, visualization, and optional recording.

\subsection{Integration with the simulation core}
\label{subsec:gui_integration}

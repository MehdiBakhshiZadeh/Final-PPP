...
The test produced the error values listed in Table~\ref{tab:mms_operator_convergence_data}. The corresponding convergence plot is shown in Figure~\ref{fig:mms_operator_convergence}.
...
\paragraph{Obtained result.}
The measured results are listed in Table~\ref{tab:timestep_convergence_data}, and the \(L^2\)-based refinement plot is shown in Figure~\ref{fig:timestep_convergence}. The numerical results are
\[
...
\begin{itemize}
    ...
    \item nonzero interior change change,
...
\paragraph{How to run the test case.}
From the project root or from the \texttt{tests/gui\_consistency} workflow with the project paths available, execute:
\begin{verbatim}
...
\paragraph{Setup.}
The test is performed for the Gray--Scott model on the periodic square domain
\[
\Omega = [0,1)\times[0,1)
\]
...
\[
\Delta t \in \left\{0.01,\;0.0025,\;0.000625,\;0.00015625,\;3.90625\times 10^{-5}\right\}.
\]
...
\[
N=64\;(\Delta t=0.01,\;N_t=100),\quad
N=128\;(\Delta t=0.0025,\;N_t=400),
\]
\[
N=256\;(\Delta t=0.000625,\;N_t=1600),\quad
N=512\;(\Delta t=0.00015625,\;N_t=6400),
\]
\[
N=1024\;(\Delta t=3.90625\times 10^{-5},\;N_t=25600).
\]
The finest-grid reference was then interpolated onto the \(64\times64\), \(128\times128\), \(256\times256\), and \(512\times512\) grids using cubic interpolation.

The measured \(L^2\) differences are listed in Table~\ref{tab:grayscott_grid_refinement}. In both \(U\) and \(V\), the error decreases monotonically as the grid is refined:
\[
...
\begin{table}[htbp]
\centering
\caption{Gray--Scott grid-refinement differences relative to the interpolated \(1024\times1024\) reference solution.}
...
\[
p_U = 1.70, \qquad p_V = 1.63.
\]
These values are clearly positive and indicate substantial convergence toward the finest-grid reference, but they are somewhat below the ideal value \(2\). For the nonlinear Gray--Scott problem, this is not unexpected. As the patterns evolve, small phase shifts and slight spatial displacements can develop between solutions computed on different grids. Such phase differences can increase the pointwise and \(L^2\) discrepancy even when the overall morphology is converging correctly. Therefore, the measured rates should be interpreted as consistency-oriented refinement rates for a nonlinear pattern-forming PDE rather than as exact textbook spatial orders.
...
\caption{Gray--Scott grid-refinement study using the \(1024\times1024\) solution as a numerical reference. The measured \(L^2\) differences in \(U\) and \(V\) decrease monotonically as the grid is refined.}
...
These errors are small and remain fully consistent with a correctly implemented manufactured-solution workflow on the chosen grid and time-step size. Figure~\ref{fig:mms_endtoend_errV} shows the spatial error field for \(v\) at the final time. The smooth, structured error pattern is typical for a discretization error field and does not indicate instability or implementation failure.
...
\subsection{Test Case 8: Pearson-Type Pattern Sanity Check}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/verification/pattern_sanity/alpha_t100000_U.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/verification/pattern_sanity/epsilon_t100000_U.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/verification/pattern_sanity/kappa_t100000_U.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\textwidth]{figures/verification/pattern_sanity/theta_t100000_U.png}

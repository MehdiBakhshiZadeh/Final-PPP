...
\paragraph{How to run the test case.}
Run the workflow either from the project root or from \texttt{tests/gui\_consistency}. Make sure that the project paths are available. Then execute:
\begin{verbatim}
...
These errors are small for the chosen grid and time-step size. They are consistent with a correctly implemented manufactured-solution workflow. Figure~\ref{fig:mms_endtoend_errV} shows the spatial error field for \(v\) at the final time. The smooth, structured pattern is typical of discretization error and does not indicate instability or implementation failure.
